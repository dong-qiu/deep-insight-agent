...
- [~] P0 生成溯源与全链路可观测性 V1：以 `Run` / `audit_log` 为基础记录采集、分析、校验、报告、技术线索、方向映射和人工决策的实体—活动—执行者链路；P0a、P0b 已完成生产验收；P0c 容量修复部署后待复验，优化驾驶舱仍待完成。见 `docs/plan/specs/generation-provenance.md`。
  ...
  - [~] P0c 容量与受限视图已完成生产规模匿名测量；event-ref 图查询计划修复部署后待复验（2026-08-17）。证据见 `docs/verify/p0c-production-capacity-acceptance-2026-08-17.md`。
...
### DCP-4 上线 / MVP 收口门评 — 2026-06-16

- **决策**：✅ 收口（MVP 达成）—— M4 Launch 收口，转入 post-MVP 迭代。**（负责人 + 架构师双签通过，2026-06-16）**
- **依据**：
  - **功能完整**：`product-definition.md`「MVP 包含」五行（采集 / 洞察 / 报告 / 账号 / UI）全部实现，**无未实现项、无打折项**（逐行核码见 `docs/verify/mvp-gap-2026-06-07.md` 总表与 §一.5）。报告库筛选**六维**（主题 / 类型 / 行业 / 来源 / 标签 / 实体）全部有数据支撑——筛选骨架（#33）+ 标签上游 analyzer 抽取（#36）。
  - **真机验证齐全（生产 AWS EC2 `i-061dc19d7f7ff81ad`，2026-06-15）**：成本熔断（强制低额 → 硬熔断 + 飞书告警）、deep_dive 六段（`rep_fe25487b` 六段 + GFM 表）、报告推送（飞书 deep-link 卡）、hover 预览、筛选六维（`rep_e4835cfc` 43 标签、下拉点亮）逐项验证通过；生产跑最新代码、容器持续健康、数据源持续采集。详见 `mvp-gap §五`。
  - **M4 / DCP-3 附条件全闭合**：成本口径 relay 实账定稿 + 预算熔断在线（ADR-0003）；失败告警飞书端到端 + 报告推送；定时 eval job（relay 可达实测）；卷备份 + 异地 S3 DR（真机验证）；initial_digest 冷启动。
- **残留（post-MVP / 非收口门）**：① 域名 + TLS（用户决策暂不购，IP 直连足够，`deploy.sh` Caddy 代码就绪）；② 视频字幕 / ASR spike（低优，产品定义已划 MVP 外）；③ Sonnet 降本 + Anthropic 列表价干净测算（外部阻塞，待直连 `sk-ant-` key）；④ 自助注册 / 多账号隔离、头脑风暴 / 蓝军 / 知识图谱 / 趋势预测（产品定义显式推后，差异化护城河留 post-MVP）。
- **本轮收口关键产出（2026-06-15/16）**：报告库筛选六维补齐（#33 / #36）+ 文档同步（#34 / #35 / #37）；生产逐项真机验证，并**暴露"生产在跑约 06-13 旧代码（合入 main ≠ 上线）"**、code-only 重部署修正；修复 #33 误提交进 main 的 `node_modules` 软链（#38）；成本上限临时放行当日 cron 后自动恢复。教训入 `practice-log.md` + 项目 memory `verify-check-deployed-version-first`。
- **战略备忘**：MVP 收口 = `product-definition.md:97`「端到端跑起来 + 个人 dogfood 闭环可持续」达成，**不等于功能完整**。下一阶段优先级 = 差异化护城河深化（可溯源一致性已成雏形；主题持续聚合已有实体 / 标签 / 主题页承载面）+ dogfood 驱动迭代。
- **双签**：负责人 dongqiu ✓ 收口确认（2026-06-16）· 架构师 ✓ 会签同意（2026-06-16）。
