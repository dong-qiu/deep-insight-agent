...
- [~] 部署上线（**产物就绪 + 冒烟验证通过**：Docker compose `up --wait` exit 0、app healthy / cron 运行，修了 2 处 healthcheck 阻断，2026-05-29；**CD workflow 已备**（`deploy.yml`：SSH 部署、build-on-server、健康门，待配 SSH secrets）；**待真正部署到生产服务器**——域名/TLS/反代（`ops/aws/deploy.sh` Caddy 自动 HTTPS 代码已完整，配 `APP_DOMAIN` 即生效，卡用户域名+DNS）；**卷备份 + off-box DR ✅ 已闭合并真机验证 2026-06-13**：①同卷——`ops/backup-db.mjs` cron 容器每日 18:00 UTC 在线备份（`db.backup()` 一致 DB + `reports/` 正文）→ `/data/backups/<ts>/` 留 14 份 retention；②异地——host cron 每日 18:30 UTC `aws s3 sync → s3://deep-insight-backups-<acct>/ec2/`（桶加固：阻断公开/版本控制/AES256/生命周期 90·30；实例角色最小 S3 策略），一次性搭建 `ops/aws/setup-dr.sh`，见 `operations.md §6.1`。EC2 真机重建镜像后均已验证产出/上传）
- [x] 文档与使用指南（README 简介 + `docs/launch/operations.md` 部署运维手册：env 速查 / 中转站 Opus 约束 / 监控 / 备份恢复 / 故障排查）
- **DCP-3 附条件（M4 内闭合）**：① 成本：含校验口径定稿 / Sonnet 降本仍待直连 key；**无 key 降本已先落两项 ✅**（2026-06-07）——(a) 跨批一致性缓存 `consistency_cache`（relay 抖动重跑 / 报告重生成复用已判定、省重复 Opus 校验，只缓存成功判定，见 `db/consistency-cache.ts`）——经独立 review 硬化：按「模型+prompt」版本隔离（改模型自动失效）、TTL 14 天（重跑可纠错）、`CONSISTENCY_CACHE=0` 开关、缓存写失败绝不污染判定；(b) `PROMPT_CACHE=0` 治"中转站只写不读 prompt cache"白付溢价（eval 实测 r/w 0/17135）；② **失败告警 ✅ 完全闭合**（2026-06-07）：`notifyFailure` 多渠道 adapter——按 `ALERT_WEBHOOK` 自动识别渠道并翻译 payload（**飞书默认主推** / ntfy / slack / discord / generic，飞书可选加签）；经两轮独立 AI review 加固（never-throw 兜底 / host 点边界 / ntfy 空 topic 守卫 / probe 解析飞书 code）；**真实飞书 webhook 端到端验证、手机收到告警**。见 `src/lib/runtime/alert.ts`、`operations.md` §12。Bark / PushDeer 推后。**定时 eval job ✅ 落地**（`.github/workflows/eval.yml` 周跑 + 手动；`run-a1.ts` 阈值门 + baseline 回归对照，失败复用渠道推告警）——**用 relay 凭据即可、不需直连 key**；待用户配 repo secrets（`ANTHROPIC_API_KEY`/`ANTHROPIC_BASE_URL` 等）+ 验证 relay 对境外 GitHub runner 可达（不可达则走 self-hosted runner / 容器内 `/api/eval`，见 `operations.md` §13）；③ 产出 yield/质量迭代——**引用覆盖 rule 4 补引已强化 ✅**（◐ 降幅待下轮 dogfood 测）、isCompleteStatement 名词结尾放宽（待取证）、跨源综合提升；④ **initial_digest 冷启动已实现 ✅**（topic 无历史报告 → 首版综述 + 宽窗/多条，`reportPlan`）/ 视频字幕 spike（低优、未做）。
