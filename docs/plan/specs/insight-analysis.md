...
7.3. P0 原子洞察：新的 analyzer 输出中，每个 `statement` 只能包含一个可被至少一条 displayed quote 直接完整支持的最小事实。`statement_citation_index` 必须显式绑定唯一 citation；两者仅可忽略 Unicode NFC、首尾/连续空白和句末标点后完全相同。该绑定在调用语义 judge 前代码 fail-closed，不能把范围、关系、机制、程度或评价先加到 statement、再交给模型“判断大致支持”。跨实验结果、原因/结论、多个比较维度、机制/部署含义必须拆成独立 insight，不能以标点或推论连接词拼接。单源但完整可证的事实优先于不完整的跨源综合；内部 `content_item_id`、arXiv 编号等不得进入展示字段。
