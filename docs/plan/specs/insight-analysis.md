...
5. 信号去噪：按重要性评分 + 阈值过滤（阈值见 `eval-criteria.md`，默认 = 3）；评分说明展示在 `importance_basis`。**过滤发生在生成 `Insight` 前**，`AnalysisBatch.insights` 中不出现 `importance < 阈值` 的条目。某窗口无重要事件时输出 `no_significant_event=true` + 空 `insights`，不凑数。P0 中模型不得生成自由文本评分理由：`importance_reason` 只能取工程选型/安全审查/评测解读/研究跟踪，服务以固定模板展示“系统重要性判断”，且必须锚定已通过的 statement 原子 claim；`importance_facts[]` 若不与 statement 完全等值即从读者投影剥离，不能成为独立改写通道。引用覆盖以**最小可验证事实**为单位：数字、实体之外，研究/来源数量、机制、比较对象、适用范围、时间、条件、因果与程度也须各有直接 quote；未覆盖的限定在 P0 必须删除或使整条洞察不发布，**不得自动补引或重写**。P1 若启用修复，仅可删除/收窄，或从同一已引 `ContentItem.body` 补逐字短引；修复后必须重跑完整展示审计。
6. 多源交叉：`source_count ≥ 2` 置 `multi_source=true`；单源结论允许输出但 `multi_source=false` 明确标注（重要结论 `importance ≥ 4` 优先选多源印证）。
7. 每条洞察必须挂 ≥1 条 `Citation`（含持久化的 `citation_ref`、`claim`、`quote` + `locator`），无引用不输出。分析端在展示级 quote 覆盖审计前必须剔除 `locator` 不可定位或缺失原子 `claim` 的 citation（标题、URL 或不在 body 的改写均不可当展示证据）；仅余下 citation 仍完整覆盖最终展示字段时才可输出，不能把无效 citation 留给下游 validator 才阻断。`statement` 是核心事实：其任一完整实质 claim（机制、范围、条件、比较、因果及程度）缺少直接 quote/span 时丢弃洞察；不得按逗号或小数点拆散带范围/条件/关系的命题，再拼接局部 quote 证明它。P0 不发布模型自由改写的 `headline`：仅与 statement 窄归一后完全等值才保留，否则置空并由消费者回退 statement；`importance_facts` 同理。span 必须记录 UTF-16 offsets 与 excerpt，并由代码验证 slice 完全一致；它只是审计定位，不能替代 judge 对完整 claim 的语义判断。可选字段失败时必须从 `verified_display` 投影剥离，不能携带未验证文字入库、报告、卡片、PPT 或历史重渲染。`statement_citation_index` 是唯一展示主引用的 1-based 持久化绑定；图谱/侧栏等 reader-visible 消费者只可读取 audited 且 `kept`/`kept_degraded` 的洞察，并要求该绑定引用的 validator verdict 为 `pass/support`，只展示该 quote。图谱实体必须逐字出现在最终 statement；不做翻译、别名扩展或模糊 NER 推断，不能确定则不生成节点/边或 entity drill。历史缺少此绑定或审计的数据标记为 legacy，不能静默视为通过。citation claim 只界定其 quote 能证明的事实，不能替代 quote；审计结果须以 citation_ref、模型/提示词版本和输入哈希持久化。每个候选必须有唯一终态及逐 claim 覆盖决策。剔除后须按留下 citation 的实际 `source_id` 重算 `source_count` / `multi_source`，不得保留虚高的多源印证标签。
...
7.3. P0 原子洞察：新的 analyzer 输出中，每个最终展示 `statement` 只能包含一个可被至少一条 displayed quote 直接完整支持的最小事实。`statement_citation_index` 必须显式绑定唯一主 citation；statement 与该 citation 的 claim 仅可忽略 Unicode NFC、首尾/连续空白和句末标点后完全相同。无效索引、空 claim 或不相等都在调用语义 judge 前 fail-closed；这不是审计后的自动改写或补引。对数字、百分比及 ASCII 专名等跨语言稳定锚点，代码还要求它们实际出现在**绑定 citation 的唯一 displayed quote**，不可从第二条 citation 拼接；中文译名等非稳定词面不走这一规则以免误杀。绑定通过后，主 validator 与独立的 coverage 模型必须都只依据该 citation 的 claim、displayed quote 与 locator 判定支持；coverage 模型不可看到正文、标题或主审结论。泛称（如 the agent、it、the gap、the effect）不得被借正文/标题/主题具体化为系统、类别、机制、比较或部署范围。任一个不支持、超时、结构非法或缺失 verdict 都拒绝候选，并持久化两者的模型、提示词版本、输入哈希、span 与 verdict。跨实验结果、原因/结论、多个比较维度、机制/部署含义必须拆成独立 insight，不能以标点或推论连接词拼接。单源但完整可证的事实优先于不完整的跨源综合；内部 `content_item_id`、arXiv 编号等不得进入展示字段。
