...
7.3. P0 原子洞察：新的 analyzer 输出中，每个最终展示 `statement` 只能包含一个可被至少一条 displayed quote 直接完整支持的最小事实。`statement_citation_index` 必须显式绑定唯一主 citation；代码在创建 `Insight` 时直接以该 citation 的 `claim` 构造最终 `statement`，避免要求模型重复两份文本而让草稿成为添加范围、关系、机制、程度或评价的旁路。无效索引或空 claim 仍在调用语义 judge 前 fail-closed；这不是审计后的自动改写或补引。绑定通过后，主 validator 与独立的 coverage 模型必须都只依据该 citation 的 claim、displayed quote 与 locator 判定支持；coverage 模型不可看到正文、标题或主审结论。任一个不支持、超时、结构非法或缺失 verdict 都拒绝候选，并持久化两者的模型、提示词版本、输入哈希、span 与 verdict。跨实验结果、原因/结论、多个比较维度、机制/部署含义必须拆成独立 insight，不能以标点或推论连接词拼接。单源但完整可证的事实优先于不完整的跨源综合；内部 `content_item_id`、arXiv 编号等不得进入展示字段。
