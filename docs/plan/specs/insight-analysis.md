...
> 2026-09 混合展示补充（ADR-0033）：以下早期 P0 规约中的“仅整合不翻译”“claim 永不驱动读者文本”
> 限制仍适用于规范 `statement`、原文证据和图谱；日报另可展示经审计并哈希绑定的 `reader_statement`。
> 中文主题的零汉字 claim 仅在原始审计通过后做一次有界翻译，并对译文重跑完整门；不允许借翻译
> 修复不被证据支持的原始事实。失败诊断和验收详见 [日报失败诊断与中文结论守卫](daily-reader-language-diagnostics.md)。

1. 主题聚合：跨源、跨语言整合同主题信息，归并近义说法。MVP 仅整合不翻译；读者可见的事实句保留绑定原文 quote 的原始语言与字节，受控 UI 标签和重要性模板可跟随 `Topic.language`，但不得把事实翻译/改写后当作证据。Daily Brief 的输入选择在存在近期（默认 48 小时）候选时，保留至少 40% 的近期项；其余名额仍按相关性与来源多样性填充，避免只追逐新而丢失必要上下文。
...
5. 信号去噪：按重要性评分 + 阈值过滤（阈值见 `eval-criteria.md`，默认 = 3）；评分说明展示在 `importance_basis`。**过滤发生在生成 `Insight` 前**，`AnalysisBatch.insights` 中不出现 `importance < 阈值` 的条目。某窗口无重要事件时输出 `no_significant_event=true` + 空 `insights`，不凑数。P0 中模型不得生成自由文本评分理由：`importance_reason` 只能取工程选型/安全审查/评测解读/研究跟踪，服务以固定模板展示“系统重要性判断”，且必须锚定已通过的 statement 原子 claim；`importance_facts[]` 一律不能成为读者事实句或独立改写通道，会从 v6 读者投影剥离。引用覆盖以**最小可验证事实**为单位：数字、实体之外，研究/来源数量、机制、比较对象、适用范围、时间、条件、因果与程度也须各有直接 quote；未覆盖的限定在 P0 必须删除或使整条洞察不发布，**不得自动补引或重写**。P1 若启用修复，仅可删除/收窄，或从同一已引 `ContentItem.body` 补逐字短引；修复后必须重跑完整展示审计。
6. 多源交叉：`source_count ≥ 2` 置 `multi_source=true`；单源结论允许输出但 `multi_source=false` 明确标注（重要结论 `importance ≥ 4` 优先选多源印证）。
7. 每条洞察必须挂 ≥1 条 `Citation`（含持久化的 `citation_ref`、内部审计 `claim`、`quote` + `locator`），无引用不输出。分析端在展示级 quote 覆盖审计前必须剔除 `locator` 不可定位或缺失原子 `claim` 的 citation（标题、URL 或不在 body 的改写均不可当展示证据）；仅余下 citation 仍完整覆盖最终展示字段时才可输出，不能把无效 citation 留给下游 validator 才阻断。`statement` 是核心事实，且 v6 读者投影中必须逐 UTF-16 code unit 等于其唯一绑定 citation 的 `quote`；因此引用以外的机制、范围、条件、比较、因果和程度不可能借翻译、摘要或标题进入读者文本。`claim` 只作生成/审计的边界记录，可以使候选被拒绝，但永不驱动 reader-visible statement、实体、图谱、报告或重要性事实。P0 不发布模型自由改写的 `headline`，一律置空并由消费者回退 source quote；`importance_facts` 同理。span 必须记录 UTF-16 offsets 与 excerpt，并由代码验证 slice 完全一致；它只是审计定位，不能替代 judge 对完整 quote 的语义判断。可选字段失败时必须从 `verified_display` 投影剥离，不能携带未验证文字入库、报告、卡片、PPT 或历史重渲染。`statement_citation_index` 是唯一展示主引用的 1-based 持久化绑定；图谱/侧栏等 reader-visible 消费者只可读取 `display_projection_version=source_quote_v1`、audited 且 `kept`/`kept_degraded` 的洞察，并要求该绑定引用的 validator verdict 为 `pass/support`、审计中 index/ref 与 statement/quote SHA-256 均相符、独立 self-contained 复核为 support；只展示一次该 quote。读路径还必须拒绝非空 headline、importance_facts 或不在受控词表中的 importance_basis；报告、图谱、技术线索、机会和 PPT 都不得从已隐藏的 audit-only citation、source_count/multi_source 或自由 LLM 润色复活其他事实。图谱实体必须逐字出现在最终 statement；不做翻译、别名扩展或模糊 NER 推断，不能确定则不生成节点/边或 entity drill。历史数据（包括 v5 audited 数据）标记为 legacy，不能静默视为通过。审计结果须以 citation_ref、投影/模型/提示词版本、输入哈希、statement/quote 哈希持久化。每个候选必须有唯一终态及逐 claim 覆盖决策。剔除后须按留下 citation 的实际 `source_id` 重算 `source_count` / `multi_source`，不得保留虚高的多源印证标签。

   - **候选完整性优先于投影可读性**：为防止模型用无关但单独可读的 quote 替换原候选来逃过审计，候选草稿/claim 的稳定数字、ASCII 专名和类别锚点仍必须由绑定 quote 本身覆盖；不一致即拒绝。这个内部 veto 不会把草稿文字带入 reader-visible 字段。
...
7.2. 受控重要性锚点降级：固定 `importance_reason` 只能使用模型显式给出的、且已通过覆盖审计的 statement 锚点；P0 不让自由 headline 成为锚点。若同一输出还含越界、重复或未通过锚点，可仅删除这些无效锚点并持久化降级原因；不得自动选择未声明的替代 claim。没有剩余通过锚点时，重要性判断与候选一并拒绝。
7.3. P0 原子洞察：新的 analyzer 候选必须显式选择唯一 `statement_citation_index`，并以 atomic `claim` 记录其选择意图；空 claim、无效索引、草稿复合 claim 或草稿与 claim 不一致都在调用语义 judge 前 fail-closed。通过后服务只把该 binding 的逐字 `quote` 投影为最终 `statement`，而不是自动翻译/改写或补引。对数字、百分比及 ASCII 专名等跨语言稳定锚点，代码仍要求候选的绑定 quote 亲自包含它们，不能从第二条 citation 拼接；中文译名等非稳定词面不走这一规则以免误杀。主 validator 审计 quote 与内部 claim 的边界；**独立 coverage 模型只接收 quote 和 locator**，不接收 claim、正文、标题、主题、其他引用或主审结论，判断其是否能被读者独立理解。quote 内的专有名、产品/基准名或“具名词 + 明确类别”本身可作为自足主体，不必借标题补分类；但只可按 quote 明说的范围展示。泛称或定指（如 the agent、it、the gap、the effect、former/latter、respectively，以及它/其/该/此/这）不得借展示外上下文具体化为系统、类别、机制、比较或部署范围；有任何未解析核心指代即拒绝。任一个不支持、超时、结构非法或缺失 verdict 都拒绝候选，并持久化两者的模型、提示词版本、输入哈希、span 与 verdict。跨实验结果、原因/结论、多个比较维度、机制/部署含义必须拆成独立 insight，不能以标点或推论连接词拼接。单源但完整可证的事实优先于不完整的跨源综合；内部 `content_item_id`、arXiv 编号等不得进入展示字段。
...
- 中文读者结论按 ADR-0033 与上述补充实施；其他目标语言的有界翻译暂未纳入。
