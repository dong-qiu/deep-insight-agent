...
7. 每条洞察必须挂 ≥1 条 `Citation`（含 `claim`、`quote` + `locator`），无引用不输出。分析端在展示级 quote 覆盖审计前必须剔除 `locator` 不可定位或缺失原子 `claim` 的 citation（标题、URL 或不在 body 的改写均不可当展示证据）；仅余下 citation 仍完整覆盖最终展示字段时才可输出，不能把无效 citation 留给下游 validator 才阻断。`statement`、非空 `headline`、`importance_basis` 均按实质 clause 判定，**每一个**机制、范围、条件、比较、因果以及“首次/上线/行业影响/独立研究/模型范围”等限定都要有直接 quote 支撑；每项 judge 结果还必须指向至少一条对应 citation claim/quote，缺项、重复/越界映射或任一项不支持均丢弃该洞察。citation claim 只界定其 quote 能证明的事实，不能替代 quote。剔除后须按留下 citation 的实际 `source_id` 重算 `source_count` / `multi_source`，不得保留虚高的多源印证标签。
