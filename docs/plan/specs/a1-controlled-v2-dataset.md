...
5. 一致性集至少 100 对，`not_support` 至少 40 对并覆盖 exaggeration、out_of_context、misattribution；每条必须有稳定 `id`。两位非生成者的独立 human reviewer 对同一 `(id, statement, source_text)` hash population 完成盲标，第三位 human 只裁决分歧。受控 `consistency-label-receipt` 必须校验两份提交、最终 JSONL、分歧裁决和分布下限的 hash 绑定；v2 lock 只可引用 `eligible_for_lock` receipt，不能用一句“已双标”的描述替代。
   ...
   - 候选规划须以至少 50 条诊断性负例意图为目标，并把负例位置跨每个来源的输入交错分配；不得把负例集中在按来源排序的尾部。每一条 candidate 在进入 checkpoint 前，必须由与生成器独立、且**不接收请求 intent** 的 validator 对其与原文的关系做诊断性校准，观测关系再由调用方与请求 intent 精确比对。未通过项可在有界次数内仅在生成进程中获得其观测关系和上一版草稿后定向重写；该反馈、校准输出、重试次数、模型/Thinking/prompt provenance 只留在 diagnostic artifact，绝不可提供给盲标者。校准仅降低候选失配风险，不能替代正式 human 标签、也不能声称保证最终 40 条 `not_support`。
