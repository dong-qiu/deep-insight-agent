...
- agent 开始时必须移除 `agent-ready` 并添加 `agent-working`。因此服务重启不会再次领取已在执行、交接或阻塞的 Issue。
- 完成交接时 agent 移除 `agent-working` 与 `agent-ready`，添加 `agent-human-review` 并评论 PR、测试与 CI 证据。人类合入后才关闭 Issue。
...
| 能力 | 试点允许 | 强制约束 |
| --- | --- | --- |
| GitHub Issues | 仅读取本仓库开放 Issue；更新试点标签与评论 | fine-grained token 仅限本仓库；token 仅在 Symphony 宿主机；不写入 `WORKFLOW.md` |
...
> GitHub adapter 将 provider-native `github_api` 暴露给 Codex，但只能调用相对 REST 路径，且实际权限受 `SYMPHONY_GITHUB_TOKEN` 限制。token 必须是仅限 `dong-qiu/deep-insight-agent` 的 fine-grained token；不能使用账户级 OAuth token、classic PAT 或可访问其他仓库的 token。

## 验收标准

1. `WORKFLOW.md` 的 YAML 可由 Symphony 解析，且不含任何字面量密钥。
2. 未设置 `SYMPHONY_GITHUB_TOKEN` 或 `SYMPHONY_WORKSPACE_ROOT` 时，服务必须拒绝开始派发。
3. 只有开放且带 `agent-ready` 的 Issue 可被领取；全局并发不超过 `1`。
...
2. 创建仅限该仓库的 GitHub fine-grained token：Contents read/write、Pull requests read/write、Issues read/write、Metadata read；不授予 Actions、Environments、Administration、Secrets、Webhooks、Deployments 或其他仓库权限。在宿主机设置 `SYMPHONY_GITHUB_TOKEN`。
