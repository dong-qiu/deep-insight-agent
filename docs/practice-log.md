...
- **后续动作**: 已做：ADR-0010 全闭环——Step1（#107）+ Step2a（#108）/2b（#109）合入并部署生产（migrate 自动补 `topic.facets`/`report_index.facets` 列 + 回填 40 历史报告，生产 `t_ai_industry.facets` 校正为 `domain:ai-industry`，health 200/data fresh 核过）；(1c) 离线 eval（`evals/relevance-floor-eval.mjs`，#110）证伪 lexical、ADR 写入「关闭裁决」表 + 数字，memory [[adr0010-archetype-classification]] 同步关闭。待评估：① 语义相关性闸门（haiku 判「是否 AI 产业动态」，语言无关）= 唯一真解，触发条件（残留显著变多 / 出现第二个语言错配源）再评，eval 作可复跑基线；② 把"权威文档里背书过的方案也要先量化""拍脑袋阈值先建离线 eval""看似调参可能是机制上限"提炼进 `skills/L3-quality.md` 与 `skills/L2-workflow.md`。
