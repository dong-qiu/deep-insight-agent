...
### 2026-06-23 · ADR-0010 分类重构收官：ADR 自己写的方案也是假设——20 行离线 eval 把 (1c) 阈值证伪，挡掉一套管线

- **日期**: 2026-06-23
- **情境**: ADR-0010「分类体系重构」收尾。Step1（archetype 行为轴 #107）+ Step2a/2b（facets 取代 industry 的字段基座 #108 + 报告库迁移 #109）依次合入并 code-only 上线生产；最后处理 ADR 自己留的开放项 **(1c) 沾边离题阈值**——两轮评审都把它写成"治 openai 离题的真方案"（命中 ≥N / title 加权 / 双语关键词）。我没按 ADR 直接写，先建离线 eval（#110）用生产真样本量化，再决定建不建。
- **观察**:
  - **ADR 自己写下的"真方案"同样是假设、且因"评审两轮背书过"最难免检**：(1c) 在 ADR 里被两轮评审反复确认为"解真痛点的一刀"，最有理由直接照着实现。但它本质是一个**未在数据上验证过的相关性假设**。45 条生产真样本（t_ai_industry / openai_news 近 30 天，**全文**算分 + 人工标 on/off）一跑——**最优 lexical 变体（双语关键词 floor=1）只比现状 +2.5pp F1，且想砍的 21 条离题噪声一条没少**；想砍噪声必须抬 floor，一抬就误杀真产业新闻（floor=2 召回 −26pp，floor=3 召回崩到 42%），精度天花板 ~52%。这是 [[2026-06-19（二）两条护城河]] "证伪我亲手写下的构造安全" 再进一层：那条求证的是"我这一秒写下的论断"，这条求证的是"ADR 里评审两轮背书、我准备照着实现的方案"——**背书过的方案比临时论断更免检，恰恰更该在写第一行实现前先量化**。
  - **根因得靠数据才看清：substring 评分对这个源结构性分不开**：眼看 openai_news 离题，直觉是"加阈值/加词"。但数据暴露的根因是 **substring 评分分不开「OpenAI 做产业」vs「OpenAI 做医疗」**——两者都含 `openai`/`gpt` 实体词；且 t_ai_industry 关键词中文为主、源是英文 → 英文内容**只能命中实体名**，中文话题词全部落空；我加的双语词又被 substring 污染（`valuation`⊂e**valuation**、`sec`⊂**sec**ure）+ 漏配（"data center"≠token `datacenter`）。**任何 lexical 信号都治不了——这是评分机制的结构上限，不是参数没调好。** 延续 [[2026-06-19 报告库去重]] "诊断靠真实数据行不靠臆测"：这次仲裁的是"一个 scoring 假设到底有没有效"。
  - **必须先把前置切片部署上线，才拿得到验证 (1c) 的真数据**：(1c) 的样本依赖 t_ai_industry 这个主题真的在生产跑、攒下 openai_news 候选——而这又依赖 Step1/2 先上线。所以顺序是 **部署 Step1/2 → 生产攒数据 → 再 eval (1c)**，不是纸面先把 (1c) 设计完。"数据驱动的决策"有时要求先把能产生数据的东西部署出去。
  - **YAGNI 用数字说话、而非用直觉**：以往 YAGNI 多靠"实例只有 1 个/痛点没举证"的定性判断（[[2026-06-13 ADR ROI]] 那类）。这次是**定量** YAGNI：最优 lexical +2.5pp F1 封顶、唯一真解（语义 LLM 判断）要新增一条管线（成本+延迟+独立 eval），为这点残留（且"客户/采用类故事是否算产业"本身可争议）ROI 偏低 → 记为已知天花板、按触发条件再评。**"不建"有了可复跑的数字背书，不是拍脑袋。**
- **经验 / 教训**:
  - **ADR/spec 里"评审背书过的方案"也是待验证假设**：越是被反复确认、越有理由直接实现的方案，越值得在写第一行代码前用最便宜的方式（一个离线 eval）量化一次。求证对象从"别人的归因"→"自己的论断"→**"权威文档里背书过的方案"**，这条是该系列最难自觉的一档。
  - **拍脑袋的阈值/相关性假设，先建离线 eval 出 before/after，再决定建不建**：20 行脚本 + 一批真样本 + 人工标注，就能把"这个方向有没有用"从争论变成一张 P/R/F1 表。比"先实现一版→上线→观察几周→才发现没用"省一个数量级。
  - **看似参数问题，可能是机制结构上限**：调阈值前先问"这个评分机制理论上分得开吗"。substring 对同实体词的 on/off、跨语言关键词——是结构分不开，调参数白费。数据能把"参数没调好"和"机制到顶了"区分开。
  - **数据驱动的决策，有时要先部署能产数据的东西**：别在纸面把依赖真实运行数据的方案设计到底；先上线产生数据的前置件，再用真数据定参/证伪。
  - **YAGNI 能定量就定量**：用"最优变体 +2.5pp 封顶 / 唯一真解要建一套管线"这种数字背书"不建"，比定性直觉更站得住，也给未来"触发条件变了再评"留了可复跑的基线。
- **后续动作**: 已做：ADR-0010 全闭环——Step1（#107）+ Step2a（#108）/2b（#109）合入并部署生产（migrate 自动补 `topic.facets`/`report_index.facets` 列 + 回填 40 历史报告，生产 `t_ai_industry.facets` 校正为 `domain:ai-industry`，health 200/data fresh 核过）；(1c) 离线 eval（`evals/relevance-floor-eval.mjs`，#110）证伪 lexical、ADR 写入「关闭裁决」表 + 数字，memory [[adr0010-archetype-classification]] 同步关闭。已做（续）：三条教训提炼进 skills——"拍脑袋阈值先建离线 eval""看似调参可能是机制上限" → `skills/L3-quality.md`「假设验证：量化先于实现」；"权威文档背书过的方案也是待验证假设" → `skills/L2-workflow.md` Plan 阶段。待评估：语义相关性闸门（haiku 判「是否 AI 产业动态」，语言无关）= 唯一真解，触发条件（残留显著变多 / 出现第二个语言错配源）再评，eval 作可复跑基线。

### 2026-06-26 · 质量诊断连环证伪：数据把假设逐个溶解，定位真解后据 ROI 缓建——「不建」也是产出

- **日期**: 2026-06-26
- **情境**: ADR-0010 收尾后转「质量迭代」（"诊断生产报告质量缺口→选最痛一刀→eval 验证→上线"）。全程用生产真数据（SSM send-command 只读查 `citation_check`/`citation`/`insight`/`validation_result`）诊断，一连证伪 4 个候选缺口/修法，定位真根因与真解，据 ROI 裁「方向定案、缓建」，写成 ADR-0011（#119）+ memory [[quality-iter-citation-blocks]]。**全程未改一行产品代码——产出是「诊断 + 不建的决策」。**
- **观察**:
  - **「最大缺口」常是测量 artifact 或内容结构，不是缺陷——别停在聚合指标**：聚合数字说"跨源综合只有 1.5%"，看着像头号缺口。深挖发现：① `multi_source` 数的是 distinct feed、低估 item 级综合；② t_ai_industry 单源**定义上**不可能多源；③ 同事件多源覆盖多是**跨主题**、被架构按设计分开。一主题内的源本就互补不重叠——**是内容结构，不是 analyzer 缺陷**，强推综合只会逼出弱链接。聚合指标把我引向错的"缺口"，**读真实例才看清**。
  - **诊断要落到"读 3 条真实洞察"，不是看比率**：blocked-rate 12.3% 看着是个统一现象。拉 3 条"全屏蔽"真实洞察读下来，发现是**三种不同子模式**：Ex1 连贯论点（引用各撑分点）、Ex3 干净 roundup（三公司各一条干净引用）、Ex2 真过度声称（陈述加了没引用覆盖的综合）。前两种是**准确的好综合被 validator「每条引用对整句」粒度误拦**，只有第三种该拦。**只有读到具体例子，才把"blocked=质量差"纠正成"blocked=校验粒度误拦好综合"**——比率会骗人，实例不会。延续 [[2026-06-19 报告库去重]]"诊断落到这一条真实渲染出什么"。
  - **"简单修"被数据逐个溶解；真解往往更深、碰核心、高风险**：设计稿原本两个"简单修"——Fix 1（标题纳入可达性）离线验证只 ~3% 净回收 + 二次拦风险；Fix 2（atomize 拆原子）一看真实例会**毁掉准确的多点综合价值** + 碎片化。两个都被证伪后，真解浮现 = **per-citation 一致性粒度**（每条引用对它所声称的**分点**判而非整句）——但它碰 moat 最核心的 validator、需 analyzer 标"引用→分点"、ADR 级高风险。**容易的都不对，对的都不容易。**
  - **区分"正确性问题 vs 显示/粒度问题"决定紧迫度**：11% 洞察"丢光干净引用"听着严重。但关键澄清——这些洞察**statement 本身准确、没 ship 错的或捏造的内容**，只是被校验粒度误拦、显示时缺引用锚（〔待补引〕）。**"内容错"和"内容对但溯源显示不全"是两个紧迫度量级**：前者 moat 破了、必修；后者显示粒度局限、可缓。这个区分直接把"必须现在修"降级为"方向定案、缓建"。
  - **"不建"需要和"建"一样的证据强度**：裁"缓建 per-citation"不是怕麻烦——是摆出三条证据：① 内容准确（非正确性问题）；② 为 11% 显示粒度改 moat 最核心 validator，ROI/风险不划算；③ 73% 洞察已健康。**有数字、有触发条件（可溯源显示成优先级 / 多点洞察占比显著升 / 出现真 ship-错证据）的"不建"，才站得住、才不是拖延。** 同 (1c)「lexical +2.5pp 封顶 → 不建」。
- **经验 / 教训**:
  - **连环证伪本身就是产出**：一程下来没改产品代码，但避免了建一堆不管用（Fix 1）、甚至有害（Fix 2 atomize 毁综合价值）的东西。"我们认真查过、确认不该建 X，因为 Y"是真实交付物，价值不亚于一个 PR。
  - **别停在聚合指标，深挖到"读真实例"**：比率（multi_source 1.5% / blocked 12.3%）会把人引向错的缺口或错的归因；拉几条真实数据行/洞察读，才区分得开"缺陷 vs 内容结构 vs 校验粒度"。
  - **容易的修被证伪后，先问"真解是不是碰核心、是不是 ROI 不划算"，而非硬上**：真解常更深更险；这时"内容是否真错 + 改动碰不碰 moat 核心 + ROI"是该不该现在建的闸门。
  - **把"正确性问题"和"显示/粒度问题"分开**：决定紧迫度。准确但溯源显示不全 ≠ ship 错内容。
  - **"不建"要有证据 + 触发条件**：定量 ROI + 明确再评条件，让缓建可追溯、可在条件变化时自动复活，而非永久遗忘。
- **后续动作**: 已做：ADR-0011（#119）记诊断 + 根因（validator `:282` 整句粒度误拦多点综合）+ 三选项裁决（Fix1/Fix2 弃、per-citation 方向定案缓建）+ 触发条件；memory [[quality-iter-citation-blocks]] 钉住张力 + 别重走的被证伪简单修（跨源综合 / atomize / 标题 / headline-coverage 健康）。待评估：① per-citation 一致性粒度（触发条件满足再做，analyzer 标注引用→分点 + validator 改粒度，A1 eval 重保 fabrication=0）；② 顺手小项「防 analyzer 过度声称」（治 Ex2 型、过 eval）；③ 把"诊断落到读真实例""区分正确性 vs 显示粒度""不建要有证据+触发条件"提炼进 `skills/L3-quality.md`。

### 2026-06-30/07-01 · 我的"本地验证"给了假绿：模拟没走真代码路径；独立 review 证伪的是我的**验证**本身

- **日期**: 2026-06-30 ~ 07-01
- **情境**: 巡检生产发现安全源 `src_project_zero` 每轮 ingest 失败、0 采集——PZ 搬站后新 feed 13MB，撞 `readTextCapped` 8MB 上限抛错。我改了两段（`readTextCapped` 加 opt-in 截断 + `parseRss` 兜底），本地对真 feed"验证"通过后提交，走独立 review。review 抓到一个 **BLOCKER**：我的修复对目标 feed **根本无效**。修正 + 加固后二审 APPROVE、合入（PR#142）、部署验证生产 0→4。
- **观察**:
  - **我的"本地已验证"是假绿——因为模拟用的不是真代码路径**：我下结论"截断到 8MB 能解析 4 篇"，靠的是一段 `matchAll(/<entry>…<\/entry>/)` 正则——它对不完整的尾条**自然丢弃**，看着完美。但生产 `parseRss` 用的是 **fast-xml-parser**，对截断的 CDATA 会**抛错**（`CDATA is not closed`）。我拿一个"行为和生产不一样的便捷近似"当验证，结论方向性错误：真跑生产代码是 0 采集、和 bug 前一模一样。**模拟的保真度不够，比不模拟更危险——它发的是假通行证。**
  - **独立 review 这次证伪的不是我的代码、是我的验证**：延续本系列"review 证伪我亲手写下的论断"（[[2026-06-19（二）两条护城河]]→[[2026-06-23 ADR-0010]]"ADR 背书过的方案也是假设"），但这条更隐蔽——前几次求证的是"论断/设计"，这次 reviewer 实测**真 fast-xml-parser × 真 13MB feed，21/21 切点全抛**，戳穿的是"我说我验证过了"这句话本身。我甚至把错误前提（"parseRss 用正则会丢半条"）写进了代码注释当依据。**"我验证过"和"我用和生产一致的路径验证过"是两回事；review 的最高价值是查后者。**
  - **"源审计"第三次栽在同一处：可达 ≠ 可采**：这个 bug 的 6-27 审计写着"feed 正常 200/10 条、0 采集是低频预期"——只验了 HTTP 可达，没复现生产 `fetchRss` 的抓取全路径。生产有三道关：robots（Changelog 全站禁爬）、WAF/IP 段拦（FreeBuf 阿里云盾）、**8MB `readTextCapped` cap**（这次）。本地 curl/probe 直 fetch 三关全绕过，看着"能采"实则采不了。**同一类教训第三次实锤（robots→WAF→8MB），已升级为硬规矩写进 memory。**
  - **部署前先核"生产真在跑哪版"，省下一次冗余部署**：改完准备 code-only 重部署，先按老规矩查了运行容器——发现**并行会话已经把含此修复的 main 部署上去了**（镜像 08:34 重建、bundle 里有 `repairTruncatedFeed`）。于是跳过部署、直接单源 on-demand collect 验证（0→4）。多会话并行推进时，"合入≠上线"还要再加一句"上线的可能不是我干的"——[[verify-check-deployed-version-first]] 的动作（查镜像/bundle 而非假设）在并行下更值钱。
- **经验 / 教训**:
  - **模拟/probe 必须走与生产一致的代码路径，否则是假绿**：用 `matchAll` 近似 `fast-xml-parser`、用 `curl` 近似 `fetchRss`——任何"行为不同的便捷替身"都可能发假通行证。要么调真函数，要么先证明替身在关键行为上等价。宁可不模拟（老实说"没验证"），也别拿不等价的模拟当"已验证"。
  - **review 的头号价值是查"验证是否成立"，不只是查代码风格**：让 reviewer 复现"你说你验证过的那步"，比让他读 diff 更能挡住 blocker。我把假设写进注释当依据这件事，也说明**自证的依据链要显式写出来给人打**。
  - **接入/审计任何外部源，先复现生产抓取全路径（robots + WAF/IP + 8MB cap），只验"可达 200/N 条"必漏判**：三次实锤后的硬规矩。
  - **并行开发下，"上线的那版可能不是我部署的"**：验证前查运行产物（镜像 created / bundle 内容），不光防"合入≠上线"，还防"别人已上线、我却又部署一遍"。
- **后续动作**: 已做：PR#142（`readTextCapped` truncate + `parseRss` `repairTruncatedFeed` + 根类型最先标签判 + CDATA 平衡切点，配套单测覆盖"截断 mid-CDATA→保留完整条""无完整条→照抛保持可见"）合入并（经并行部署）上线，生产实测 `src_project_zero` 0→4；memory 新增 [[oversized-feed-8mb-cap]]（含"源审计必复现生产抓取全路径"硬规矩）；7/3 被引复盘 routine 已补 PZ 特例（防误判下线）。待办：把"模拟必须走真代码路径、否则算未验证""review 优先复现验证步"提炼进 `skills/L3-quality.md`。

### 2026-07-01 · 重测反转搁置结论：信备忘 vs 真跑数——一个验证 bug 恰好演示了被验证方案的坑

- **日期**: 2026-07-01
- **情境**: 质量提升计划 6 片收尾，回看昨天（6-30）「先量证伪」搁置的 Q5（analyzer 诚实信号：① 截断检测 ② 数字 token 词边界修）。我先给了基于代码 + 昨天备忘的判断（维持搁置、子项②"1 行随手带"），并主动提出两条路：(a) 只按更正刷备忘；(b) 走 SSM 拉生产库把两个数**重测一遍**。用户选 (b)。重测的结果**推翻了搁置时的数字依据，且把子项②的处置从"随手带"反转成"明确否决"**。
- **观察**:
  - **备忘里的数字会过期/失真——真跑数才是地面真值**：昨天备忘写"shipped 789 洞察 / 数字误匹配 1/1103=0.09% / 纯展示零风险"。SSM 直读 `/data/insight.db` 实测：shipped 是 **696**（非 789）；且读代码发现 `coverageGaps` 不止喂显示、还喂 `repairCoverage`(analyzer:179) 的 LLM 补引 → **非纯展示、动它要过 eval-gate**。备忘是「写下那刻的快照」，"memory 里点名的文件/数字，推荐前先核"这条纪律这次真兑现了价值——三处失真（数字、口径、风险定性）全靠回到代码 + 生产数据才纠正。
  - **我的验证脚本先跑出一个错数，而这个错恰好现场演示了被验证方案本身的坑**：第一版测量 word-boundary 误匹配率跑出 **10/813=1.23%**（13× 于备忘的 0.09%）。数字反常 → 没有直接采信，而是逐条 dump `hay` 上下文。真相：`coverageGaps` 的 `norm` **先剥空白再 `includes`**，原文 "Fable 5 80.3%" 剥空白粘成 "Fable**5**80.3%"，把前面的 "5" 糊到 "80.3%" 上、伪造出"数字边界冲突"。**这 9 条百分比本就被正确覆盖——而我的测量 bug（在剥空白后的串上判边界），正是 word-boundary 方案真上线后会犯的同一个错**：它会把这 9 条正确覆盖误判成缺口、冒出假 〔待补引〕。**验证过程的 bug 意外成了对被验证方案的反例。**
  - **在"原文"上重核，把 artifact 和真信号分开**：改成在**保留空白的原文**上判数字边界，真·有害误匹配从 10 塌到 **1/813=0.12%**，且唯一那条是 `12.5` 命中 `$12.50/million` 的子串——**良性**（"$12.5" 与 "$12.50" 同值，现判"已覆盖"是对的；word-boundary 反而会误标成缺口）。结论因此比昨天更硬且方向相反：**现行 substring-includes 在真实数据上比 word-boundary 更准，"修复"是净负、会造 bug。** 延续 [[2026-06-26 质量诊断连环证伪]]"别停在聚合指标、深挖到读真实例"——这次读的是"每个被判误匹配的 token 在原文里到底长什么样"。
  - **截断检测子项：0% 之外还有更强的"已被上游拦"论据**：截断结尾非终结标点实测 **0/696=0%**；更关键是截断的 LLM 输出会先撞 **JSON parse 失败 → 整条洞察被丢**，是比字符串启发式**更强的上游拦截**。为一个"生产 0 发生 + 上游已强拦"的失败模式再建一层启发式 = 给已灭的火装烟感。
- **经验 / 教训**:
  - **搁置/结论所依赖的数字，隔一段时间要能"真跑数"复核，别让备忘变成免检真值**：备忘是快照、会失真（数字、口径、风险定性都可能过期）。涉及"要不要做/做不做得对"的决策依据，回到代码 + 生产数据重测一次的边际价值，常常不是"确认结论"而是"发现结论方向错了"。
  - **验证脚本自身先要被怀疑——尤其当它跑出反常数字时**：反常的测量结果，先假设是测量错、逐条落到真实例核对，而不是直接采信。这次的 1.23% 是测量 artifact；若直接信它，反而会"论证"出一个错误的高误匹配率去支持一个有害的修法。
  - **注意"归一化顺序"制造的假信号**：先 `norm`（剥空白/fold）再做边界/子串判断，会把原本分开的 token 粘连，制造出原文里不存在的边界冲突。判"边界/相邻"关系时，要在**保留原始分隔的文本**上判，别在压扁后的串上判。
  - **"修复"要先证明它比现状更准，而非默认现状是 bug**：substring-includes 看着"不严谨"，但在这个数据 + 这个 `norm` 管线下它恰好更鲁棒；word-boundary 的"严谨"反而误杀。改一个看似糙的实现前，先在真实数据上量"改完是更准还是更差"。
- **后续动作**: 已做：SSM 生产实测两个数（截断 0/696；真·有害误匹配 1/813=0.12% 且良性），逐条 dump 上下文定位 `norm` 剥空白粘连 artifact；memory [[quality-audit-and-plan]] 的 Q5 行 + 描述 + 索引全部更正（789→696、0.09%→实测口径、"随手带"→"否决/净负"、补注 `coverageGaps` 喂补引非纯展示）。至此质量计划 6 片定案：Q1-Q3 已部署 / Q4+Q6 合入待随批部署 / **Q5 实测否决** / 唯一遗留 scheduler 197 行拆分缓做。待评估：把"备忘数字要能真跑数复核""验证脚本自身先被怀疑""归一化顺序制造假信号""改糙实现前先量更准还是更差"提炼进 `skills/L3-quality.md`。
