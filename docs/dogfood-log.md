...
- 2026-06-06 23:xx · cron 频率 Path A 落地：ops/crontab 从 `0 */6 * * *`（每 6h）改 `0 17 * * *`（每日 UTC 17:00 = 北京 01:00），早晨用户起床可读当日 brief；下次触发今晚 7.5 小时后。注释更新："源 fetch_interval=6h 是采集层意愿、与 cron 触发频率无关"——上面 spec 漏洞"配置 schema 设计 papermark 第 2 例"已闭合。元教训："实现配置"和"产品契约"的口径不一致是隐性 bug——cron 配的是"6h"但用户读的是"每日 Brief"，需要在 spec / 注释 / UI 三处保持一致表述
- 2026-06-06 23:xx · rep_af85ba6b 深度报告（以及其他几份）打开显示"正文文件缺失，请重新生成"——但**文件实际存在**（**根因 = body_path 相对路径跨环境失效**）：①诊断 4 份 5 月报告 body_path 写的是 `.data/reports/rep_xxx`（相对路径），容器内 cwd=/app，getReport 读 `/app/.data/reports/rep_xxx.md` → 不存在；但真实文件在 `/data/reports/rep_xxx.md`，36 个 .md 齐全。②修两路：a) DB UPDATE `REPLACE(body_path, '.data/reports/', '/data/reports/')` 4 行修复，8 份报告全可读；b) saveReport 用 `path.resolve()` 始终把 body_path 写成绝对路径，加单测守护契约。③这是 5/31 practice-log "worktree 相对 DB 路径陷阱"的同根复发——元教训："任何跨进程/跨环境持久化的路径，存的瞬间必须绝对化"应写进 L0 foundation skills
- 2026-06-06 · 比较深度报告 vs Brief / spec 设计原则 #2"不复报：跨日 brief 应能感知昨日 event 而不重复" —— 实际**未实现**：selectInsights 无历史去重、analyzer 不喂上下文，每日 brief 互相独立（**P1 落地**：①DB insight 表加 is_followup INTEGER 列 + ensureColumn 兜底旧库；②`Insight` 类型 + `LlmInsightSchema` 加 `event_id: nullable / is_followup: boolean`；③analyzer SYSTEM prompt 增 "跨日不复报" 规则 30 行（同事件复用 event_id + 设 is_followup=true + 无新进展整条不出）；④user prompt 末附"近 14 天该主题 brief 已报告事件清单"（新增 `listRecentBriefEvents` 查 brief 报告 join insight 表，同 event_id 取最新、deep_dive 不入、上限 50）；⑤白名单防 LLM 编造 event_id 字符串——LLM 回传的 id 必须在历史清单里才采用，否则即便 is_followup=true 也降级为 false + 新 event_id；⑥新事件代码侧分配 `evt_${batchId}_${i}`；⑦report-gen md 渲染 〔更新〕 / html 渲染 `<span class="followup">更新</span>`；724 测试全过，新增 5 测试。spec 漏洞元教训：spec 写了"不复报"但未给实施路径——"事件对齐 / event_id 复用"这种**跨调用状态**功能必须显式标"骨架阶段不做、后续 PX 落地"，否则容易留半句承诺。后续 P2 = 让 deep_dive 真正"深"（独立 SYSTEM + Executive Synthesis 段）
- 2026-06-06 · P1 端到端验证：rebuild docker + 两次 cron 真模型链路跑通（**P1 主体落地证据齐全**）：①cron #1 种子：50 个洞察 event_id 100% 走 evt_batch_*、followup 全 0；②`listRecentBriefEvents` 真把 50 个种子 id 喂进 cron #2 的 analyzer user prompt；③LLM 真复用——cron #2 产 9 条 followup（6 t_code_agents + 3 t_prompt_injection），event_id 全部回指 cron #1 种子（白名单 0 例非法）；④report-gen 5 条纳入的 followup 真渲染 〔更新〕 标记；从「100% 重复 → 零自感知」到「9/9 followup 全部正确指认」=**spec 原则 #2 从 0% → 主体落地**。⑤副产物 bug：中转站 2026-06-06 新策略对 forced tool_choice + thinking 组合返 400 "Thinking may not be enabled when tool_choice forces tool use"——callStructured 必定 forced tool_choice、validator 默认 thinking=true → 整条 validate 100% 失败成 〔待核实〕。修法：llm.ts callStructured 不再发 thinking 参数（opts.thinking 语义层保留，等中转站放开或切直连再恢复）。⑥剩余空间（不阻塞）：prompt 写"无新进展整条不出"，LLM 仍出 2/3 条 importance_basis 自承"无实质新进展"的 followup——服从度问题、不影响事件对齐准确性，未来可加 few-shot 反例或代码侧相似度过滤。元教训：**dogfood 真模型验证 ≠ 单测**——单测保证"代码不破"，但 LLM 服从度、中转站策略变化、跨调用状态串联这些只能真跑两次 cron 看到。验证档 1（看清单）→ 档 2（一次 cron）→ 档 3（跨日观察）的分层是对的
