...
- **方法**：拉 t_code_agents top-6 共现对的洞察 headline 逐对读判有向关系信号。
- **发现**：① 清楚的有向关系几乎只有 **org→产品「发布/开发」**（Anthropic→Fable 5）——**从实体类型即可推**（org+product 共现≈org 造 product），不值得 LLM 抽；② 高共现对主导是**「并列/对比」**（一堆编码 agent 一起被评测）——**共现本身已表达**，抽成「compared-with」边零增信息；③ **真高价值有向关系（M&A/投资/依赖/对立）在本领域稀疏**，样本里几乎没有。
- **裁决**：用**薄+冗余+可推导**的信号，喂一个**高 LLM 成本 + 高幻觉风险（假关系 ship=破护城河）+ 改分析管线过 A1** 的抽取——不划算。**搁置**（同 ADR-0009/0011「先量证伪、别建被证伪的」）。
- **再评触发**：出现**真需求信号**（如领域转向产业并购、大量 M&A/投资事件入库），或 dogfood 明确反馈「需要有向关系」。届时优先**窄域**（只抽 M&A/投资/合作、强 grounding 防幻觉），而非全量。

### S2 触发条件（再评类型化语义边）
S1 dogfood 证图有用 + 确需有向语义（谁收购谁 / 谁对立谁）→ 才投 LLM 关系抽取（届时过 A1，保 `fabrication_rate=0`）。**当前先量评估为 ROI 低、已搁置，见上。**
...
## ADR-0014: Daily Brief 的近期证据门与采集/出刊解耦
...
生产快照显示采集每天正常完成，选择池也含当天内容，但 Daily Brief 可因相关性排序、LLM 聚合与校验结果而
只发布数日前的证据，造成“今日”内容与实际时间脱节。此前每日 cron 同时承担采集和出刊，`fetch_interval`
没有独立触发语义。

### 决定

1. Daily Brief 选择阶段在存在近 48 小时候选时保留至少 40% 近期输入（`BRIEF_FRESH_HOURS` /
   `BRIEF_FRESH_QUOTA` 可配置）；仍保留其他名额给相关历史上下文。
2. 发布阶段加确定性闸门：若已选到近期候选，每条可见洞察都必须含一条该候选集的成功校验引用；否则输出
   done 状态的“无重要事件”，不重放旧证据。该门与 event_id 的“新增证据才复报”规则叠加。
3. `report_index` 记录最新候选、最新有效引用和时滞，历史报告保持 NULL，不伪造监控数据。
4. cron 每日 05/11/23 UTC 仅采集，17 UTC 完整管线先采后出刊；保持每天一份 Brief，避免重复 LLM 成本。

### 验证与退出条件

- 选择器、报告生成、持久化和 cron 分流都须有生产路径测试，并运行 typecheck；本改动不修改 prompt、模型、
  校验逻辑或评测集，因此按 Eval-Gate 走确定性路径验证，不以 A1 评测代替。
- 观察新报告的 `freshness_lag_hours`；若低频主题频繁空刊，优先校准主题/源覆盖或近期窗口，不降低引用校验门。

---

## ADR-0015: 技术线索由成功校验洞察确定性派生

- **日期**: 2026-07-23
- **状态**: Accepted

技术线索不能成为绕过引用白名单的第二条发布链。V1 只消费 `verdict=pass` 的洞察引用，标题与理由均由
已有洞察字段确定性投影，推荐分数可拆解且不额外调用 LLM。相同 canonical key 仅在出现新增成功证据时更新。

这样先验证“用户是否愿意关注/深挖线索”的产品价值；一周 dogfood 后，才评估需要为 LLM 富化引入何种专用评测和校验。

---

## ADR-0016: 技术线索之上增加技术机会与方向校准层
...
技术线索 V1 能可靠回答“发生了什么技术变化”，却不能回答它是否服务于当前主题的研究 / 开发方向；仅按新鲜度、来源数和重要性排序也会让新资讯压过可验证的长期技术机会。另一方面，完全按既定方向过滤会错过用于修正方向的相邻、远期和反证信号。

### 决定

1. 保留 `TechLead` 作为事实层；新增 `TopicDirection`、映射和 `TechnologyOpportunity` 作为规划层。机会只能链接已有 Lead，证据读取仍回到 `CitationCheck(pass)`。
2. 方向关系显式分为 `core` / `adjacent` / `horizon` / `challenge`。前两类服务已有方向；后两类服务方向校准，单独展示且不自动创建方向。
3. 首版使用方向档案中的显式匹配词和确定性评分，输出“待验证假设”和“建议验证动作”；人工才能采纳、创建方向或升级到项目候选，所有决定写审计日志。
4. 机会优先级以方向匹配、证据成熟度、技术杠杆、可验证性和时机计算；新鲜度降为时机维度，避免新闻流主导技术规划。
5. 语义 LLM 映射、来源等级和概念聚类必须先有 50–100 条人工标注样本及独立评测；不得把现有 A1 当作该能力的质量证明。

### 后果

主题从关键词路由单元演变为可人工维护的技术组合，但系统仍只提供决策材料，不自动采取外部行动。新增确定性投影不改 analyzer / validator prompt 或模型，按 scoped Eval-Gate 验证生产路径；后续 LLM 阶段必须走完整 Eval-Gate。
...
## ADR-0021: Daily Brief 采用“近期主通道 + 未发布补充发现”选择策略
...
Daily Brief 的输入选择会保留近期候选，但报告层在任一近期候选存在时要求每条洞察均引用近期候选。生产溯源显示，
分析与引用校验均成功的主题仍频繁落为 `no_new_publishable_insight`：模型可合理地使用窗口内较早的可信上下文，
却被这个更强的输出门整体过滤。

### 决定

1. 保留近期成功校验证据作为主发布通道，以及既有 14 天 event / 新增成功证据去重规则。
2. 对不带近期成功校验证据的洞察，仅允许带稳定 `event_id`、从未出现在 14 天发布基线、且至少有一条
   `pass/support` 白名单引用的项目作为补充发现；报告仍只渲染白名单引用。每期最多两条，并在 Markdown/HTML 中标记
   `〔补充发现〕`。无 event id 的较早趋势不走此旁路。
3. 选择过程在 generation trace 中单独记录补充候选和补充发布计数，管理员面板将计数渲染为日报漏斗；失败告警、
   源熔断和源健康可视化复用既有链路。
...
`source_quote_v1` 将读者正文完全投影为绑定 quote，强化了逐字可追溯性，但英文来源会直接成为日报
正文；同时，单条原文投影隐藏了原先的中文结论和可扫读摘要。读者仍可访问来源链接，却难以在日报中
快速判断洞察，且孤立的 `[N]` 编号没有对应展示锚点。

### 决定

1. 保持 `Insight.statement` 为精确绑定的原文 quote，继续作为来源事实、去重与图谱的稳定锚点；不以
   翻译或摘要替换该证据。
2. 仅保存已通过既有展示覆盖审计的投影前草稿为 `reader_statement`。它在中文 topic 中是中文原子结论，
   其 hash 必须等于审计中的 `draft_statement_sha256`，并且该审计的 statement claim 与独立 countercheck
   均为 support；任一条件不满足即不展示该字段，回退 quote。
3. 新日报以 `reader_statement` 作为结论，紧随绑定 quote、来源链接和日期；一个引用编号必须对应一个
   可达的证据项，悬浮预览与键盘跳转使用同一目标。次级支持引用继续留在审计面，不伪装为对该单一
   中文结论的独立复述。
4. 卡片摘要与 highlights 仅在上述 reader statement 存在且通过绑定时恢复；不得从原始抓取、标题或
   任意历史 statement 临时生成中文文案。
