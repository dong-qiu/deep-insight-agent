...
const publishStatus = publishVerifiedLocalPair(
  { path: disputesPath, bytes: Buffer.from(`${compared.disputes.map((row) => JSON.stringify(row)).join("\n")}${compared.disputes.length ? "\n" : ""}`), label: "AI dispute worklist" },
  { path: receiptPath, bytes: Buffer.from(`${JSON.stringify(compared.receipt, null, 2)}\n`), label: "AI comparison receipt" },
);
console.log(`AI 对比${publishStatus === "recovered" ? "恢复" : "完成"}：${compared.disputes.length}/100 条分歧已写入无标签 human dispute worklist；receipt=${receiptPath}`);
