...
const context: CandidateCheckpointContext = {
  ...
  calibration_prompt_sha256: "calibration", calibration_max_generation_attempts: 3, calibration_drafts_per_attempt: 3,
};
...
describe("consistency candidate checkpoint", () => {
  it("persists only complete prefix batches and rejects input/configuration drift", () => {
    ...
    expect(() => loadCandidateCheckpoint(path, { ...context, calibration_drafts_per_attempt: 2 }, plan)).toThrow("不匹配");
