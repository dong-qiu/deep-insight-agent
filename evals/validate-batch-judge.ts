/**
 * 批量一致性判定精度验证（PR #51 上线前门）：把 citation-consistency 标注集
...
/** Load configuration before importing runtime modules, whose MODELS constant is resolved at import time. */
export function loadEnvLocal(path = ".env.local", env: NodeJS.ProcessEnv = process.env): void {
...
export interface BatchJudgeCase { statement: string; source_text: string; expected_consistency: "support" | "not_support" | "uncertain"; negative_type?: string }
type V = ConsistencyJudge["consistency"] | "error";

/** Keep run evidence tied to the dataset that was actually loaded, not a stale prose count. */
export function describeBatchJudgeDataset(cases: BatchJudgeCase[]): {
  caseCount: number;
  ...
  return {
    ...
    negativeCount: cases.filter((c) => c.expected_consistency === "not_support").length,
    uniqueSourceCount: new Set(cases.map((c) => c.source_text)).size,
  };
}

/** `tsx evals/file.ts` supplies a relative argv[1]; normalize it before comparing module URLs. */
export function isDirectBatchJudgeExecution(moduleUrl: string, entryPath: string | undefined): boolean {
...
function metrics(verdicts: V[], cases: BatchJudgeCase[]): { acc: number; negRecall: number; negTotal: number; negMissed: number[]; errors: number } {
...
async function main(): Promise<void> {
  loadEnvLocal();
  // Do not turn these back into static imports: their model configuration must see .env.local.
  const [{ judgeBatchWithRetry, judgeWithRetry }, { assertModelSeparation, getCostReport, MODELS }] = await Promise.all([
    import("../src/lib/agents/validator.js"),
    import("../src/lib/runtime/llm.js"),
  ]);
  assertModelSeparation();
  const CONC = Math.max(1, Number(process.env.EVAL_CONCURRENCY ?? 6));
  const cases = readJsonl<BatchJudgeCase>("evals/dataset/citation-consistency.jsonl");
  const dataset = describeBatchJudgeDataset(cases);
  console.log(`validator=${MODELS.validator} · thinking=${process.env.VALIDATOR_THINKING !== "0" ? "on" : "off"} · 并发=${CONC}`);
  console.log(`样本：${dataset.caseCount} 例（负例 ${dataset.negativeCount}；来源 ${dataset.uniqueSourceCount} 个）\n`);
  ...
  console.log("\n指标         单条        批量");
  console.log(`三分类准确率  ${pct(mS.acc).padEnd(12)}${pct(mB.acc)}`);
  console.log(`负例召回率    ${pct(mS.negRecall).padEnd(12)}${pct(mB.negRecall)}  ← 安全门 ≥95%`);
  ...
  console.log(`\n成本：$${cost.totalUSD.toFixed(4)}（单条 ${dataset.caseCount} + 批量 ${groupList.length} 调用）`);

  // 判定
  const safe = mB.negRecall >= 0.95 && newMiss.length === 0;
  console.log(`\n结论：${safe ? "✅ 本诊断范围内，批量不弱于单条且负例召回守住" : "❌ 批量出现精度回归 → 暂不上线，排查"}`);
  process.exit(safe ? 0 : 1);
}

if (isDirectBatchJudgeExecution(import.meta.url, process.argv[1])) {
