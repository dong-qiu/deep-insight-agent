...
const publishStatus = publishVerifiedLocalPair(
  { path: finalPath, bytes: finalBytes, label: "AI-assisted chat final JSONL" },
  { path: receiptPath, bytes: Buffer.from(`${JSON.stringify(finalized.receipt, null, 2)}\n`), label: "AI-assisted chat final receipt" },
);
console.log(`${publishStatus === "recovered" ? "恢复" : "已写入"} ${finalized.cases.length} 条 prototype_ai_assisted 标签（human_with_ai_advice；lock_eligible=false）：${receiptPath}`);
