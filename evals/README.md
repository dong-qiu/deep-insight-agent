...
| 展示引用覆盖 unsafe_accept | 手标 `dataset/display-coverage-benchmark.json`；先验证 statement→唯一 citation claim 的显式绑定及绑定 quote 内稳定 token，再由主 validator 与独立 coverage 模型分别验证 claim→displayed quote/span；任一 reject 被放行即 FAIL，任一复核错误/缺 verdict 也拒绝。P0 headline/importance_fact 只允许完全复用 statement | 自动 |
