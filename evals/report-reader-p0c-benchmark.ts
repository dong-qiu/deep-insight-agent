...
function validateFixture(): void {
  if (fixture.version !== 3 || !positiveInteger(fixture.warmup_samples) || !positiveInteger(fixture.measurement_samples)
    || !positiveInteger(fixture.measurement_rounds)
...
try {
  ...
  const baselineP95Rounds = [] as number[];
  const currentP95Rounds = [] as number[];
  for (let round = 0; round < fixture.measurement_rounds; round += 1) {
    ...
    for (let index = 0; index < fixture.measurement_samples; index += 1) {
      ...
      if ((round + index) % 2 === 0) { baseline.push(measure(baselineReader)); current.push(measure(currentReader)); }
  ...
  const result = {
    ...
    measurement_rounds: fixture.measurement_rounds,
