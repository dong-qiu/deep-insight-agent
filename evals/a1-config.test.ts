...
const current: EvalConfig = {
  ...
  coverage_model: "coverage", validator_thinking: false, coverage_thinking: false, coverage_thinking_source: "explicit", coverage_max_tokens: 2048, structured_thinking_transport_version: "forced-tool-enabled-v1", validator_batch: true,
...
describe("A1 config comparability", () => {
  it("requires every contract fingerprint, including the analyzer output version and prompt", () => {
    ...
    expect(sameEvalConfig({ ...current, coverage_max_tokens: 4096 }, current)).toBe(false);
