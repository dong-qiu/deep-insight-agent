...
const current: EvalConfig = {
  ...
  display_coverage_gate_version: "display-coverage-v5", display_coverage_primary_prompt_version: "v5",
  display_coverage_primary_prompt_sha256: "primary", display_coverage_countercheck_prompt_version: "v2",
...
describe("A1 config comparability", () => {
  it("requires every contract fingerprint, including the analyzer output version and prompt", () => {
    ...
    expect(sameEvalConfig({ ...current, display_coverage_gate_version: "display-coverage-v6" }, current)).toBe(false);
