  ...
  if (disputes.length) {
    if (!adjudication) issues.push("AI 分歧存在但缺少 human adjudication");
      ...
      for (const decision of adjudication.decisions) {
        if (adjudicated.has(decision.case_id)) issues.push(`human adjudication 重复决定 ${decision.case_id}`);
        adjudicated.set(decision.case_id, decision);
        if (expected.get(decision.case_id) !== decision.pair_sha256) issues.push(`human adjudication 的 ${decision.case_id} pair hash 不匹配`);
        if (!validDecision(decision)) issues.push(`human adjudication 的 ${decision.case_id} 标签或 negative_type 无效`);
      }
      for (const id of expected.keys()) if (!adjudicated.has(id)) issues.push(`human adjudication 缺少 AI 分歧项 ${id}`);
    }
