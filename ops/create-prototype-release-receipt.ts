...
const [safetyPath, ciPath, dockerPath, output] = process.argv.slice(2);
if (!safetyPath || !ciPath || !dockerPath || !output) {
  throw new Error("用法：tsx ops/create-prototype-release-receipt.ts <prototype-safety.json> <prototype-ci-evidence.json> <prototype-docker-evidence.json> <out.json>");
}
...
const receipt = createPrototypeReleaseReceipt({
  ...
  dockerEvidence: JSON.parse(readFileSync(dockerPath, "utf8")),
});
