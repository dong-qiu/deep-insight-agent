...
PREV_TRANSCRIPT_SHADOW="$(extract_prev TRANSCRIPT_SHADOW_FETCH)"
...
# —— 运行时配置块：继承旧值则原样保留，否则写注释占位（运维按需填）——
{
  ...
  echo "# 播客转写全局应急允许门：当前 observer 不会写入 production ContentItem；默认关闭。"
  echo "${PREV_TRANSCRIPT:-# TRANSCRIPT_FETCH=0}"
  echo "# 隔离 shadow 样本：1=observe 源写 /data/podcast-shadow；默认关闭，不影响生产 ContentItem。"
  echo "${PREV_TRANSCRIPT_SHADOW:-# TRANSCRIPT_SHADOW_FETCH=0}"
