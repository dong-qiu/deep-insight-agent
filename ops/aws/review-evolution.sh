#!/usr/bin/env bash
# 主题演化第一步（ADR-0005 / PR #42，2026-06-16 上线）回看：只读核对生产 t_code_agents 的演化数据。
...
cat > "$PARAMS" <<EOF
{"commands":["echo '=== /api/health ==='; curl -fsS http://127.0.0.1:3000/api/health 2>/dev/null; echo; echo '=== 演化数据核对 ==='; docker exec -w /app deep-insight-app-1 sh -c 'printf %s $B64 | base64 -d > /tmp/q.js && NODE_PATH=/app/node_modules node /tmp/q.js $SINCE'"]}
EOF
