...
const shell = readFileSync(fileURLToPath(new URL("./set-pragmatic-transcript-observe.sh", import.meta.url)), "utf8");
const payload = shell.match(/read -r -d '' migration_js <<'JS' \|\| true\n([\s\S]*?)\nJS\n/)?.[1];
if (!payload) throw new Error("could not extract Pragmatic observe payload");
const source: Source = {
  id: "src_pragmatic_eng", name: "The Pragmatic Engineer", type: "rss", endpoint: "https://newsletter.pragmaticengineer.com/feed",
  topic_ids: ["t_code_agents"], fetch_interval: "24h", backfill: null, enabled: true, fetch_mode: "feed", content_container: null,
...
let db: DB;
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "ia-pragmatic-observe-"));
  db = openDb(join(dir, "insight.db")); insertSource(db, source); db.close();
});
...
it("only transitions the exact audited source off -> observe and is idempotent", () => {
  ...
  expect(policy()).toEqual({ transcript_mode: "observe", transcript_strategy: "relevant_only", transcript_max_items_per_run: 1,
...
it("refuses a drifted source or an enabled policy rather than overwriting it", () => {
  ...
  mutate.prepare("UPDATE source SET endpoint=? WHERE id=?").run("https://example.invalid/feed", source.id); mutate.close();
  expect(runPayload().status).toBe(1);
  const reset = openDb(join(dir, "insight.db"));
  reset.prepare("UPDATE source SET endpoint=?, transcript_mode='enabled' WHERE id=?").run(source.endpoint, source.id); reset.close();
  expect(runPayload().status).toBe(1);
});
