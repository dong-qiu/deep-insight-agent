...
const mocks = vi.hoisted(() => ({ actor: vi.fn(), db: vi.fn(), audit: vi.fn(), store: vi.fn(), verify: vi.fn(), notify: vi.fn() }));
...
vi.mock("../../../../../../lib/db/integrity-checks.js", () => ({ verifyArtifactIntegrity: mocks.verify }));
...
describe("POST /api/admin/integrity-checks/:artifact/:version", () => {
  ...
  it("notifies only for an integrity failure", async () => {
    mocks.verify.mockResolvedValue({ artifact_id: "artifact-1", artifact_version: "v1", outcome: "anchor_mismatch", failure_step: "anchor", expected_hash_prefix: null, actual_hash_prefix: null, checked_at: "2026-08-22T00:00:00Z" });
