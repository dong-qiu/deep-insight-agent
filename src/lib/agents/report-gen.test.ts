...
describe("selectInsights（洞察级纳入判定）", () => {
  ...
  it("仅保留明确 support 的引用；blocked、flagged 均不发布", () => {
    expect(sel.map((x) => x.insight.id)).toEqual(["i1"]); // i2 仅 uncertain、i3 全 blocked，均被排除
...
describe("selectBriefInsights（Daily Brief 已发布证据去重）", () => {
  ...
  it("同 event 且没有新增成功校验证据时，缓存洞察不得再次发布", () => {
    ...
    expect(selected.map((x) => x.insight.id)).toEqual([]);
  ...
  it("deep_dive 不套用 Daily Brief 去重基线", () => {
    expect(selectBriefInsights(batchOf(), validation, "deep_dive", published).map((x) => x.insight.id)).toEqual(["i1"]);
  });
...
describe("buildReport 派生", () => {
  ...
  it("正文仅含明确 support 洞察，flagged 只留在人工核实队列", () => {
...
describe("buildReport · deep_dive（最小确定性深挖）", () => {
  ...
  it("按重要性分节 + 节内三级标题", () => {
    expect(report.body_md).toContain("## 重点关注"); // i1 importance≥4
...
describe("flagLabel + 校验失败/待核实 区分（C）", () => {
  ...
  it("#2：pass + 校验失败 → 仍纳入（靠 pass），但错误引用不展示", () => {
    const sel = selOf([chk(0, "pass", "support"), chk(1, "flagged", "not_evaluated")], 2);
    expect(sel).toHaveLength(1);
    expect(sel[0].citationIndices).toEqual([0]);
    expect(sel[0].flaggedError).toBe(false);
    expect(sel[0].flaggedUncertain).toBe(false);
  });

  it("#4：genuine uncertain + 校验失败并存 → 无明确 support，不发布", () => {
    const sel = selOf([chk(0, "flagged", "uncertain"), chk(1, "flagged", "not_evaluated")], 2);
    expect(sel).toHaveLength(0);
  });

  it("防御异常数据：verdict=pass 但 consistency 非 support → 不发布", () => {
    expect(selOf([chk(0, "pass", "uncertain")], 1)).toHaveLength(0);
  });
});
