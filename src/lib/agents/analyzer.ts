...
/** 分析缓存版本（ADR-0009）：analyzer 模型 + SYSTEM prompt 哈希——改模型/改 prompt → 版本变 → 旧分析缓存
 *  自动失效（不复用陈旧 prompt 的洞察）。镜像 validator.consistencyCacheVersion 的版本隔离口径。
 *  ⚠️ 切片1（只写不读·只度量）下仅哈希 SYSTEM 足够、无害；**切片2（据缓存跳过重析）落地前须把
 *  AnalyzerOutputSchema + 代码侧派生逻辑也纳入版本哈希**，否则改 schema 不改 SYSTEM 会错命中陈旧洞察（review m3）。 */
export function analyzerCacheVersion(): string {
  const promptHash = createHash("sha256").update(SYSTEM).digest("hex").slice(0, 12);
  return `${MODELS.analyzer}|${promptHash}`;
}
