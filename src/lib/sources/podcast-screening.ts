...
export function screenPodcastCandidate(raw: RawItem, topics: Pick<Topic, "id" | "keywords">[]): PodcastScreeningDecision {
  ...
  if (!topics.length) return { candidate_hash, decision: "unknown", reason_code: "missing_topic_profile", matched_topic_ids: [] };

  const corpus = lower([raw.title, raw.author, raw.body].filter(Boolean).join("\n"));
  if (corpus.trim().length < 8) {
    return { candidate_hash, decision: "unknown", reason_code: "insufficient_metadata", matched_topic_ids: [] };
  }
  ...
  if (matched_topic_ids.length) {
    return { candidate_hash, decision: "fetch", reason_code: "topic_keyword_match", matched_topic_ids };
  }
  if (raw.podcast_episode_type === "trailer") {
    return { candidate_hash, decision: "hard_negative", reason_code: "trailer_without_topic_match", matched_topic_ids: [] };
  }
  return { candidate_hash, decision: "unknown", reason_code: "no_keyword_high_recall", matched_topic_ids: [] };
}
