...
import { forbidNonAdmin, requireAdminActor } from "./auth-guard.js";

afterEach(() => vi.mocked(auth).mockReset());

describe("forbidNonAdmin（二道鉴权闸）", () => {
  it("admin → 放行", async () => {
    vi.mocked(auth).mockResolvedValue({ user: { role: "admin", id: "u1", email: "a@example.com" } } as never);

    await expect(forbidNonAdmin()).resolves.toBeNull();
    await expect(requireAdminActor()).resolves.toEqual({ id: "u1", role: "admin" });
  });
