...
export function rowToInsight(db: DB, r: InsightRow): Insight {
  ...
  return {
    ...
    ...(r.reader_statement?.trim() ? { reader_statement: r.reader_statement } : {}),
...
export function saveAnalysisBatch(db: DB, batch: AnalysisBatch, afterSave?: () => void): void {
  db.transaction(() => {
    ...
    const insStmt = db.prepare(
      `INSERT INTO insight
         (id,batch_id,topic_id,type,event_id,statement,reader_statement,statement_citation_index,headline,importance,importance_basis,source_count,multi_source,time_window,confidence,language,is_followup,entities,tags)
       VALUES (@id,@batch_id,@topic_id,@type,@event_id,@statement,@reader_statement,@statement_citation_index,@headline,@importance,@importance_basis,@source_count,@multi_source,@time_window,@confidence,@language,@is_followup,@entities,@tags)`,
    );
    ...
    for (const ins of batch.insights) {
      insStmt.run({
        ...
        statement: ins.statement, reader_statement: ins.reader_statement ?? "", statement_citation_index: ins.statement_citation_index ?? null,
