...
describe("provenance migration runner", () => {
  ...
  it("applies once, records its checksum, and is safe to rerun", () => {
    ...
    expect(db.prepare("SELECT COUNT(*) AS count FROM schema_migration").get()).toEqual({ count: 46 });
    ...
    expect((db.prepare("PRAGMA table_info(insight)").all() as { name: string }[]).some((row) => row.name === "reader_statement")).toBe(true);
