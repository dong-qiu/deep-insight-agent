...
// Step2c 砍 industry 的 guard 级回归：驱动 openDb→migrate() 的"industry→facets 回填 + DROP COLUMN + 二次幂等"全链路。
// 用 ALTER ADD COLUMN 把 Step2c 库**降级回"带 industry 列"的 pre-Step2c 形态**（facets 清空），再 openDb 触发迁移。
function columnNames(db: DB, table: string): string[] {
  return (db.prepare(`PRAGMA table_info(${table})`).all() as { name: string }[]).map((c) => c.name);
}
describe("Step2c 砍 industry 迁移（自包含：回填 facets + DROP COLUMN）", () => {
  it("pre-Step2c 库（带 industry 列、facets 空）openDb 后从 industry 回填 facets 并删列；二次 openDb 幂等", () => {
    const file = join(dir, "step2c-migrate.db");
    // 1) 正常建库（Step2c schema：无 industry 列）+ 落一条 ai-security 报告。
    const db1 = openDb(file);
    insertTopic(db1, { id: "t_sec", name: "SEC", keywords: ["k"], facets: ["domain:ai-security"], language: "zh", brief_schedule: "daily", enabled: true });
    // t_ind 模拟 t_ai_industry：industry 是 ai-swe，但 facets 已在 Step2b 手工校正为 domain:ai-industry——
    // 回填**必须跳过它**（WHERE facets IN ('[]','')），否则会被错误覆盖成 domain:ai-swe（最危险的回归）。
    insertTopic(db1, { id: "t_ind", name: "IND", keywords: ["k"], facets: ["domain:ai-industry"], language: "zh", brief_schedule: "daily", enabled: true });
    insertSource(db1, { id: "src_x", name: "X", type: "rss", endpoint: "https://e.com/f", topic_ids: ["t_sec"], fetch_interval: "1h", backfill: null, enabled: true, fetch_mode: "feed", content_container: null });
    saveReport(db1, { ...report, id: "rep_legacy", topic_id: "t_sec" }, { ...index, report_id: "rep_legacy", topic_id: "t_sec", facets: ["domain:ai-security"] }, { dir });
    // 2) 降级到 pre-Step2c：三表加回 industry 列 + 写值；t_sec/report 的 facets 清空（模拟"只有 industry"的存量行），
    //    t_ind 的 facets 保留 domain:ai-industry（industry=ai-swe，模拟 Step2b 已校正、回填须跳过）。
    for (const t of ["topic", "report_index", "source"]) db1.exec(`ALTER TABLE ${t} ADD COLUMN industry TEXT`);
    db1.exec("UPDATE topic SET industry='ai-security', facets='[]' WHERE id='t_sec'");
    db1.exec("UPDATE topic SET industry='ai-swe' WHERE id='t_ind'"); // facets 保持 domain:ai-industry
    db1.exec("UPDATE report_index SET industry='ai-security', facets='[]'");
    db1.exec("UPDATE source SET industry='ai-security'");
    db1.close();
