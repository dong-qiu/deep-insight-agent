...
import { commitAnchoredPublication, planAnchorPublication, reconcileAnchoredEffects, writeDailyMerkleRoot, writePlannedAnchor } from "./integrity-publication.js";
...
describe("P1c publication visibility and recovery", () => {
  it("keeps reports invisible when the final SQLite transaction fails, then reconciles the same anchor", async () => {
    ...
    expect(() => commitAnchoredPublication(db, { manifest: input.manifest, generation_effect_id: input.generation_effect_id, provider_version_id: null, finalize: () => { throw new Error("sqlite_commit_failure"); } })).toThrow("sqlite_commit_failure");
    ...
    expect(await reconcileAnchoredEffects(db, store, () => undefined)).toEqual({ reconciled: 1, failed: 0 });
  ...
  it("treats a network-unknown write as an exact retry and detects orphan conflicts", async () => {
    ...
    expect(await reconcileAnchoredEffects(db, store, () => undefined)).toEqual({ reconciled: 0, failed: 1 });
  ...
  it("builds a daily root without changing already committed reader state", async () => {
    const db = seeded(); const input = { generation_effect_id: "effect-1", manifest: manifest(), issued_at: "2026-08-21T00:00:01Z", retain_until: "2027-01-01T00:00:00Z" }; const store = new MemoryAnchorStore();
    await writePlannedAnchor(db, store, input, signer);
    commitAnchoredPublication(db, { manifest: input.manifest, generation_effect_id: "effect-1", provider_version_id: null, finalize: () => db.prepare("UPDATE report SET status='done' WHERE id='report-1'").run() });
