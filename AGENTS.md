...
- 创建 linked worktree 后，从主 worktree 复制所需的 gitignored 本地运行配置（目前为 `.env.local`），并将权限收紧为仅所有者可读写；不得输出、提交或把密钥写入版本化文件。绝不复制 `.data/`、SQLite/WAL/报告产物或 `.env.development.local`；若复制的配置含 `DB_PATH`/`DATA_DIR`，必须改为目标 worktree 的隔离路径后才能运行。
